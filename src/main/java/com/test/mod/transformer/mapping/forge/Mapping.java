package com.test.mod.transformer.mapping.forge;
...
public class Mapping {
    ...
    static {
        try {
           mappingFile = IMappingFile.load(new File(Preloader.MAIN_PATH + "\\forge_mapping.tsrg"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static String getInternalName(String originalName) {
        if (isNotObfuscated()) return originalName;

        IMappingFile.IClass mappingClass = mappingFile.getClass(originalName);
        if (mappingClass != null && mappingClass.getMapped() != null) {
            return mappingClass.getMapped();
        }
        return originalName;
    }
    ...
    public static String test(Class<?> clazz, String notObfuscatedName, String docs) {
