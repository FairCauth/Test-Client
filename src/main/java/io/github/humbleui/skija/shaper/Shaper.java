...
public class Shaper extends Managed {
    ...
    public static Shaper makeShaperDrivenWrapper(@Nullable FontMgr fontMgr) {
        try {
            Stats.onNativeCall();
            return new Shaper(_nMakeShaperDrivenWrapper(getPtr(fontMgr)));
        } finally {
    ...
    public static Shaper makeShapeThenWrap(@Nullable FontMgr fontMgr) {
        try {
            Stats.onNativeCall();
            return new Shaper(_nMakeShapeThenWrap(getPtr(fontMgr)));
        } finally {
            ReferenceUtil.reachabilityFence(fontMgr);
        }
    }
    ...
    public static Shaper makeShapeDontWrapOrReorder(@Nullable FontMgr fontMgr) {
        try {
            Stats.onNativeCall();
            return new Shaper(_nMakeShapeDontWrapOrReorder(getPtr(fontMgr)));
        } finally {
            ReferenceUtil.reachabilityFence(fontMgr);
        }
    }
    ...
    public static Shaper make(@Nullable FontMgr fontMgr) {
        try {
            Stats.onNativeCall();
            return new Shaper(_nMake(getPtr(fontMgr)));
        } finally {
            ReferenceUtil.reachabilityFence(fontMgr);
        }
    }
    ...
    public TextBlob shape(String text, @NotNull Font font, @NotNull ShapingOptions opts, float width, @NotNull Point offset) {
        try {
            ...
            Stats.onNativeCall();
            long ptr = _nShapeBlob(_ptr, text, getPtr(font), opts, width, offset._x, offset._y);
    ...
    public TextLine shapeLine(String text, @NotNull Font font, @NotNull ShapingOptions opts) {
        try {
            ...
            Stats.onNativeCall();
            return new TextLine(_nShapeLine(_ptr, text, getPtr(font), opts));
        } finally {
            ReferenceUtil.reachabilityFence(this);
            ReferenceUtil.reachabilityFence(font);
        }
                        ...
                        @NotNull RunHandler runHandler)
    {
        try {
            ...
            assert opts != null : "Can’t Shaper::shape with opts == null";
            Stats.onNativeCall();
            _nShape(_ptr, getPtr(textUtf8), fontIter, bidiIter, scriptIter, langIter, opts, width, runHandler);
            return this;
        } finally {
