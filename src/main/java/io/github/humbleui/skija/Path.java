...
public class Path extends Managed implements Iterable<PathSegment> {
    ...
    */
    ...
    public boolean _nativeEquals(Native other) {
        try {
            return _nEquals(_ptr, getPtr(other));
        } finally {
    ...
    public boolean isInterpolatable(Path compare) {
        try {
            Stats.onNativeCall();
            return _nIsInterpolatable(_ptr, getPtr(compare));
        } finally {
            ReferenceUtil.reachabilityFence(this);
    ...
    public Path makeLerp(Path ending, float weight) {
        try {
            Stats.onNativeCall();
            long ptr = _nMakeLerp(_ptr, getPtr(ending), weight);
            if (ptr == 0)
    ...
    public Path addPath(Path src, boolean extend) {
        try {
            Stats.onNativeCall();
            _nAddPath(_ptr, getPtr(src), extend);
            return this;
        } finally {
    ...
    public Path addPath(Path src, float dx, float dy, boolean extend) {
        try {
            Stats.onNativeCall();
            _nAddPathOffset(_ptr, getPtr(src), dx, dy, extend);
            return this;
        } finally {
            ReferenceUtil.reachabilityFence(src);
        }
    ...
    public Path addPath(Path src, Matrix33 matrix, boolean extend) {
        try {
            Stats.onNativeCall();
            _nAddPathTransform(_ptr, getPtr(src), matrix.getMat(), extend);
            return this;
        } finally {
            ReferenceUtil.reachabilityFence(src);
        }
    ...
    public Path transform(Matrix33 matrix, Path dst, boolean applyPerspectiveClip) {
        try {
            Stats.onNativeCall();
            _nTransform(_ptr, matrix.getMat(), getPtr(dst), applyPerspectiveClip);
            return this;
        } finally {
            ReferenceUtil.reachabilityFence(dst);
        }
    ...
    public static Path makeCombining(@NotNull Path one, @NotNull Path two, PathOp op) {
        try {
            Stats.onNativeCall();
            long ptr = _nMakeCombining(getPtr(one), getPtr(two), op.ordinal());
